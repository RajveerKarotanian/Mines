 ...
 * 	Date: 3/24/25
 *
 * 	Description:	This program is a Mines casino games. The initial balanceis $1000, where
 * 					the balance is displayed on a frame. The frame will also give the user the 
 * 					ability to pick a number of mines on the board, and they will be able to 
 * 					enter a bet amount. An increased number of mines will result in a higher base
 * 			 		multiplier for money return. Their current profit will display on the bottom 
 * 					of the side panel. To start, the user will have to press the bet button, 
 * 					withdrawing the amount of money bet from the balance, where appropriate error
 * 	 				messages will be displayed when invalid information is entered. Pressing this
 * 				 	button will start the round, where the amount of mines picked will be scattered
 *  				across the 6 x 6 grid, where every object not a mine will be a gem. The objective
 * 		 			of this game is to pick as many gems as possible, trying to avoid clicking on a 
 * 					bomb. The amount of gems picked will add onto the multiplier on the bet amount.
 *  				The total cashout value will update every time a gem is clicked, where the user 
 * 					will be able to cashout at any point in the game as long as atleast one gem is 
 * 					clicked. If a bomb is clicked before the user is able to cashout, they will lose 
 * 					all the money. Upon cashing out or a bomb blowing up, the entire screen will be 
 * 					revealed. The side panel, will become interactive again, allowing the user to bet 
...
public class Mines extends JFrame implements ActionListener{
	...
	private double bet, gemCount, numBombs, total, balance;
	...
	public Mines() {
		...
		balanceAmount = new JTextArea(formatter.format(balance));
	...
	private void updateProfit() {
		if (gemCount != 0) {
			total = bet * Math.max(0.6, Math.pow(1.1 + (numBombs * 0.035), gemCount) - (0.7 - (numBombs * 0.01)));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == sideButton) { // if bet pressed
			if(sideButtonText.getTitle().equals("Bet")) {
				...
				if (betAmount.getText().equals("")) { // if nothing entered
				...
				else if (Double.parseDouble(betAmount.getText()) > balance) { // if not enough money
					...
					balanceAmount.setText(formatter.format(balance));
